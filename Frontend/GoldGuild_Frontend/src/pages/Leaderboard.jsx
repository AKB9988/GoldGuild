    ...
    return(
        <div className="w-full bg-[#0F0F0F] text-zinc-400 p-6 font-sans">
            ...
            {isLoading ? (
                ...
                <>
                    <div className="flex items-end justify-center gap-6 mb-8 pt-8 select-none">
                        {rank2 && (
                            <div className="flex flex-col items-center">
                                <div className="relative mb-2">
                                <div className=" rounded-full w-12 h-12 bg-zinc-600 border border-zinc-500 flex items-center justify-center font-bold text-sm text-white shadow-md">{rank2.username.slice(0,2).toUpperCase()}</div>
                                </div>
                                <div className="w-20 h-16 bg-zinc-800/80 border border-zinc-700/50 rounded-t-lg flex flex-col items-center justify-center text-center shadow-lg">
                                    <span className="text-lg font-black text-zinc-400">#2</span>
                                    <span className="text-[10px] text-zinc-500 font-bold max-w-[70px] truncate">{rank2.username}</span>
                                    <span className="text-[9px] text-[#8A8A8A]">{rank2.xp.toLocaleString()} XP</span>
                                </div>
                            </div>
                        )}
                        ...
                        {rank1 && (
                            <div className="flex flex-col items-center">
                                <div className="relative mb-2">
                                    <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-lg animate-bounce">👑</span>
                                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-gold to-[#d97706] border-2 border-gold flex items-center justify-center font-extrabold text-lg text-[#0F0F0F] shadow-gold">
                                        {rank1.username.slice(0, 2).toUpperCase()}
                                    </div>

                                </div>
                                <div className="w-24 h-24 bg-gradient-to-t from-gold-glow to-gold/25 border border-gold-dim rounded-t-xl flex flex-col items-center justify-center text-center shadow-gold">
                                    <span className="text-2xl font-black text-gold">#1</span>
                                    <span className="text-xs text-white font-bold max-w-[80px] truncate">{rank1.username}</span>
                                    <span className="text-[10px] text-gold font-semibold">{rank1.xp.toLocaleString()} XP</span>
                                </div>
                            </div>
                        )}
                        ...
                        {rank3 && (
                            <div className="flex flex-col items-center">
                                <div className="relative mb-2">
                                    <div className="w-11 h-11 rounded-full bg-amber-800/90 border border-amber-700 flex items-center justify-center font-bold text-xs text-white shadow-md">
                                        {rank3.username.slice(0, 2).toUpperCase()}
                                    </div>

                                </div>
                                <div className="w-20 h-12 bg-amber-900/20 border border-amber-950/40 rounded-t-lg flex flex-col items-center justify-center text-center shadow-lg">
                                    <span className="text-base font-black text-amber-600">#3</span>
                                    <span className="text-[10px] text-zinc-500 font-bold max-w-[70px] truncate">{rank3.username}</span>
                                    <span className="text-[9px] text-[#8A8A8A]">{rank3.xp.toLocaleString()} XP</span>
                                </div>
                            </div>
                        )}
                                    ...
                                    leaderboard.map((user)=>{
                                        ...
                                        return(
                                            <tr key={user.rank}
                                                ...
                                                <td className="p-4 text-center font-bold text-zinc-300">Lvl {user.level}</td>
                                                <td className="p-4 text-right font-black text-gold text-sm tracking-tight">{user.xp.toLocaleString()} XP</td>
                                            </tr>
                                        )
                                    })
                                }
